...
public class CreateBackupUseCaseTests : IAsyncLifetime
{
    ...
    public async Task ExecuteAsync_WhenSnapshotWith2Files_With3FilesOnDisk_ShouldGenerate1NewFile()
    {
        ...
        Assert.Equal(currentSnapshotName, result.Value.Files[0].FoundInSnapshot);
        Assert.Equal(currentSnapshotName, result.Value.Files[1].FoundInSnapshot);
        Assert.Equal(result.Value.SnapshotName, result.Value.Files[2].FoundInSnapshot);
    ...
    public async Task ExecuteAsync_WhenSnapshotWith2Files_With3FilesOnDisk_OneDifferent_ShouldGenerate2NewFiles()
    {
        ...
        Assert.Equal(currentSnapshotName, result.Value.Files[0].FoundInSnapshot);
        Assert.Equal(result.Value.SnapshotName, result.Value.Files[1].FoundInSnapshot);
        Assert.Equal(result.Value.SnapshotName, result.Value.Files[2].FoundInSnapshot);
